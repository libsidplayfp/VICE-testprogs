...
!macro push_stat a1  {        ;setting flags in the processor status register
            lda #a1
            pha         ;use stack to load status
            }

!macro set_stat a1   {        ;setting flags in the processor status register
            lda #a1
            pha         ;use stack to load status
            plp
            }
...
nmi_count   !byte 0           ;lowest number handled first, $ff = never
irq_count   !byte 0           ;separation-1 = instructions between interrupts
brk_count   !byte 0
;expected interrupt mask
I_src       !byte 0           ;bit: 0=BRK, 1=IRQ, 2=NMI
data_bss
...
irq_f   !byte 0               ;flags
nmi_a   !byte 0               ;a register
nmi_x   !byte 0               ;x register
nmi_f   !byte 0               ;flags
zp_bss
