	...
	!initmem 	fillvalue
	!cpu 6502
	!to "keywordsv2mdbasic.prg", cbm

; All BASIC v2 + MDBASIC (C64) tokens, one per statement.
;
; MDBASIC re-uses the GO token ($CB) as "off" and adds keywords through $FE
; (see the mdbasickwcb[] table in petcat.c). $FF is the standard pi symbol.
; Every code from $80 to $FF is a valid token in this dialect, so emit them
; all, eight per line, separated by ":".
