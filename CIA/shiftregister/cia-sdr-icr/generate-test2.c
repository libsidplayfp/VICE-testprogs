...
#define OFFSET1 39  // 41 cycles between writes to CRA, minus one cycle to load TA, and stop one cycle before last write
#define OFFSET2 40
//#define OFFSET1 0
//#define OFFSET2 0
...
void reset1()
{
    ...
    if (baud < 3) {
        // Correct for TA count between counter latch and SDR latch
        ++bits;
    }
 ...
 * This models: if you 
 * - clear the ICR
 * - start the timer / set output mode
 * - and the shifter,
 * - clear the ICR
 * - wait some time (each next call 1 cycle longer),
 * - clear the ICR,
 * - stop the timer / set input mode,
...
uint8_t check1()
{
    /* If latch is 0, always set the TA and SP int bits */
    if (baud == 0) {
    ...
    if (delaysetsdr1 ||
        (bits && (bits < 14 || (bits == 14 && baud - count1 >= 4))))
    {
        expected1 |= INT_SDR;
    }
	...
	count1 = baud;	/* reload from latch */
        ...
        if (bits && !--bits) {
            /* Transmission complete. SP INT is raised 4 cycles after TA INT */
            /* Down counter to check whether SP INT occured after read of ICR */
            delaysetsdr1 = 14;
	}
    }

    if (baud - count1 <= 10) {
        /* at most 10 cycles since last TA INT (read of ICR 10 cycles ago) */
        expected1 |= INT_TA;
    }
